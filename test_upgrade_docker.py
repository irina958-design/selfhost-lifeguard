...
REDIS_IMAGE = "docker.io/valkey/valkey:9@sha256:4963247afc4cd33c7d3b2d2816b9f7f8eeebab148d29056c2ca4d7cbc966f2d9"


def create_source_install(root: Path) -> list[str]:
...
class DockerUpgradeTest(unittest.TestCase):
    def test_real_immich_patch_upgrade_rehearsal(self):
        with tempfile.TemporaryDirectory(prefix="lifeguard real upgrade ") as temporary:
            root = Path(temporary) / "source installation"
            compose_command = create_source_install(root)
            try:
                subprocess.run([*compose_command, "up", "-d", "--wait"], check=True, timeout=900)
                backup = create_database_backup(root)
                result = subprocess.run(
                    [sys.executable, str(SCRIPT), str(root), "--rehearse-upgrade", "v3.0.3"],
                    capture_output=True,
                    text=True,
                    timeout=1200,
                )
